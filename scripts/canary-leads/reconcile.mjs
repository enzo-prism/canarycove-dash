...
const HELD_SOURCE_FORMS = new Set(["typeform"])

function isHeldLead(submission) {
  return HELD_SOURCE_FORMS.has(submission?.sourceForm)
}
...
export function seedExisting({ contactRows, bookingRows, existingSubmissions, existingSummary, exportedAt }) {
  ...
  const formspreeExisting = existingSubmissions.filter((row) => !isHeldLead(row))
  const expected = {
    rawRows: operational.length,
    visibleRows: existingSubmissions.length,
    filteredRows: operational.length - formspreeExisting.length,
  ...
  for (const current of existingSubmissions) {
    if (isHeldLead(current)) continue
  ...
  if (result.summary.visibleRows !== formspreeExisting.length || result.summary.filteredRows !== existingSummary.filteredRows) {
...
export function renderSubmissionsModule(result, existingSubmissions = []) {
  ...
  const summary = {
    ...result.summary,
    visibleRows: mergedLeads.length,
  }
...
export function mergeExistingSubmissions(normalizedLeads, existingSubmissions = []) {
  ...
  for (const existing of existingSubmissions) {
    if (!existing.id || existingById.has(existing.id) || heldLeads.some((lead) => lead.id === existing.id)) {
      throw new Error(`Existing dashboard ID ${existing.id ?? "missing"} is duplicated`)
    }
    if (isHeldLead(existing)) {
      if (normalizedById.has(existing.id)) {
        throw new Error(`Held dashboard ID ${existing.id} collides with a source row`)
      }
      heldLeads.push(structuredClone(existing))
      continue
    }
  ...
  return [...heldLeads, ...normalizedLeads.map((lead) => structuredClone(existingById.get(lead.id) ?? lead))]
